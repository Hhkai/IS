...
CBigNum CBigNum::PowMod(CBigNum rhs, const CBigNum &mod, const clock_t ctShowSteps) const
{
   ...
   while (rhs > 0U)
   {
      ...
      if (ctShowSteps)
      {
         ...
         {
            printf("%d bits remaining in PowMod.\n", nSteps);
            ctCurrent = clock();
         }
      }
   }
...
CBigNum CBigNum::FromByteString(const char *szIn, const unsigned int nLength)
{
   ...
   for (i=0; i<nUseLength; i+=1)
   {
      r = nUseLength - 1 - i;
      if (i % 2)
         Result.m_arVal[i >> 1] |= ((unsigned char)szIn[r]) << 8;
      else
         Result.m_arVal[i >> 1] = ((unsigned char)szIn[r]);
   }
...
CBigNumString CBigNum::ToBase64String() const
{
   // NOTE!  Does not conform to rfc1521 because no line breaks are inserted.
   CBigNumString Result;
   unsigned int i, r;
   unsigned int nInSize = m_nSize;
   unsigned int nOutSize;
   unsigned char b[3];
   unsigned char nEqCnt = 0;
   ...
   while ((nInSize > 0) && !m_arVal[nInSize-1])
      nInSize--;
   ...
   if (m_arVal[nInSize-1] & 0xFF00)
      nInSize = nInSize * 2;
   else
      nInSize = nInSize * 2 - 1;

   nOutSize = ((nInSize+2)/3)*4;

   Result.Realloc(nOutSize+1);
   for (i=0U, r=nInSize; r--; i+=4)
   {
      b[0] = (m_arVal[r>>1] >> ((r % 2) << 3)) & 0xFF;
      
      if (r)
      {
         r--;
         b[1] = (m_arVal[r>>1] >> ((r % 2) << 3)) & 0xFF;
      }
      else
         nEqCnt++;
      ...
      {
         r--;
         b[2] = (m_arVal[r>>1] >> ((r % 2) << 3)) & 0xFF;
      }
      else
         nEqCnt++;

      Result[i] = szBase64[(b[0] & 0xFC) >> 2];
      Result[i+1] = szBase64[((b[0] & 0x03) << 4) | ((b[1] & 0xF0) >> 4)];
      Result[i+2] = szBase64[((b[1] & 0x0F) << 2) | ((b[2] & 0xC0) >> 6)];
      Result[i+3] = szBase64[b[2] & 0x3F];
   }
